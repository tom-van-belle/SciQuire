...
from RAG_Chatbot.backend.data_repository.models import UserQueryRecord, ScientificAbstract
from RAG_Chatbot.backend.data_repository.interface import UserQueryDataStore
from RAG_Chatbot.config.logging_config import get_logger
...
class LocalJSONStore(UserQueryDataStore):
    ...
    def save_dataset(self, abstracts_data: List[ScientificAbstract], user_query: str) -> str:
        ...
        try:
            ...
            with open(f"{self.storage_folder_path}/{query_id}/abstracts.json", "w", encoding='utf-8') as file:
                list_of_abstracts = [model.model_dump() for model in abstracts_data]
                json.dump(list_of_abstracts, file, indent=4)

            with open(f"{self.storage_folder_path}/{query_id}/query_details.json", "w", encoding='utf-8') as file:
    ...
    def get_abstracts_count(self, query_id: str) -> int:
        """ 
        Get the number of abstracts in the selected query.
        """
        retrieved_abstracts = self.read_dataset(query_id)
        return len(retrieved_abstracts)

    def store_answer(self, query_id: str, answer: str) -> None:
        """ 
        Store the answer string to a local file in the query folder. 
        """
        try:
            answer_file_path = f'{self.storage_folder_path}/{query_id}/answer.txt'
            # Use UTF-8 encoding to support all unicode characters (e.g., Greek letters)
            with open(answer_file_path, 'w', encoding='utf-8') as file:
                file.write(answer)
            self.logger.info(f"Answer for query ID {query_id} stored successfully.")
        except Exception as e:
            self.logger.error(f"Failed to store answer for query ID {query_id}: {e}")
            raise RuntimeError(f"Failed to store answer due to an error: {e}")

    def get_answer(self, query_id: str) -> str:
        """ 
        Retrieve the answer string from a local file in the query folder. 
        """
        try:
            answer_file_path = f'{self.storage_folder_path}/{query_id}/answer.txt'
            ...
            self.logger.error(f"Failed to retrieve answer for query ID {query_id}: {e}")
            raise RuntimeError(f"Failed to retrieve answer due to an error: {e}")
